...
pub static MAINNET_GLOBAL_PARAMETERS: LazyLock<GlobalParameters> = LazyLock::new(|| {
    ...
    GlobalParameters {
        ...
        epoch_length,
        stability_window: Slot::from((active_slot_coeff_inverse * consensus_security_param * 3) as u64),
...
pub static MAINNET_DEFAULT_PROTOCOL_PARAMETERS: LazyLock<ProtocolParameters> = LazyLock::new(|| ProtocolParameters {
    ...
    max_tx_ex_units: ExUnits { mem: 16_500_000, steps: 10_000_000_000 },
    max_block_ex_units: ExUnits { mem: 72_000_000, steps: 20_000_000_000 },
    max_value_size: 5000,
    ...
    cost_models: CostModels {
        ...
        plutus_v3: Some(vec![
            ...
            1, 33852, 32, 68246, 32, 72362, 32, 7243, 32, 7391, 32, 11546, 32, 85848, 123203, 7305, -900, 1716, 549,
            57, 85848, 0, 1, 90434, 519, 0, 1, 74433, 32, 85848, 123203, 7305, -900, 1716, 549, 57, 85848, 0, 1, 1,
    ...
    ProtocolParameters {
        ...
        cost_models: CostModels {
            plutus_v1: Some(vec![
                ...
                519, 0, 1, 74433, 32, 85848, 228465, 122, 0, 1, 1, 85848, 228465, 122, 0, 1, 1, 270652, 22588, 4,
                1457325, 64566, 4, 20467, 1, 4, 0, 141992, 32, 100788, 420, 1, 1, 81663, 32, 59498, 32, 20142, 32,
...
pub static PREVIEW_GLOBAL_PARAMETERS: LazyLock<GlobalParameters> = LazyLock::new(|| {
    ...
    let epoch_length_scale_factor = 10;
    let epoch_length = active_slot_coeff_inverse * epoch_length_scale_factor * consensus_security_param;
    let stability_window = Slot::from((active_slot_coeff_inverse * consensus_security_param * 3) as u64);
    ...
    ProtocolParameters {
        ...
        cost_models: CostModels {
            ...
            plutus_v3: Some(vec![
                ...
                267929, 18, 76433006, 8868, 18, 52948122, 18, 1995836, 36, 3227919, 12, 901022, 1, 166917843, 4307, 36,
                284546, 36, 158221314, 26549, 36, 74698472, 36, 333849714, 1, 254006273, 72, 2174038, 72, 2261318,
                64571, 4, 207616, 8310, 4, 1293828, 28716, 63, 0, 1, 1006041, 43623, 251, 0, 1,
            ]),
        },
        pool_voting_thresholds: PoolVotingThresholds {
            motion_no_confidence: RationalNumber { numerator: 51, denominator: 100 },
...
pub static TESTNET_GLOBAL_PARAMETERS: LazyLock<GlobalParameters> = LazyLock::new(|| {
    ...
    GlobalParameters {
        ...
        max_kes_evolution: 62,
        epoch_length,
        stability_window: Slot::from((active_slot_coeff_inverse * consensus_security_param * 2) as u64),
        randomness_stabilization_window: (4 * consensus_security_param * active_slot_coeff_inverse) as u64,
        system_start,
    }
});
