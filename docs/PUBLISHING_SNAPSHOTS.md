...
3. The run opens a pull request that updates `snapshots.json`. Review and
   merge it.
...
- `db-analyser` is downloaded by the workflow when missing. To use your own
  build, put it on `$PATH`.
